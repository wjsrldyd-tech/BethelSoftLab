...
export default function Page3Input() {
  const { hymn1Image, updateData } = useWorshipStore();
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        updateData({ hymn1Image: event.target?.result as string });
      };
  ...
  return (
    <div className="space-y-1 max-w-4xl">
      ...
      <div className="bg-slate-800 border-l-4 border-blue-600 p-4">
        <h2 className="text-lg font-bold text-white uppercase tracking-wide">경배와 찬양</h2>
        <p className="text-sm text-slate-300 mt-1">3페이지 악보 이미지를 업로드하세요</p>
      </div>
